...
divcol <- which(colSums(abs(data)) == 0)

expression <- data[,1:(divcol-1)] 
accessibility <- data[,(divcol+1):dim(data)[2]]
...
accessibility <- RunTFIDF(accessibility, style = parameters$style)

out <- RunSVD(accessibility, n_components = ndim)
X = out[[1]]
svd = out[[2]]
...
optpoint <- max(c(min(c(optpoint1, optpoint2)), parameters$min_dim_lsi))
} else {
optpoint <- ndim
}
...
rownames(lsi) <- rownames(accessibility)
...
# we will take more than 3 PCs only if both methods recommend it
optpoint <- max(c(min(c(optpoint1, optpoint2)), parameters$min_dim_pca))
} else {
optpoint <- ndim
}
